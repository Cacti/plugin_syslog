...
builder.dataset.tree = JSON.stringify(['predicate', 'logtime', 'last', '3600']);
context.initSyslogSearchBuilder();
const preset = builder.querySelector('.syslogSearchText');
assert.equal(preset.tag, 'select');
preset.value = '604800';
context.$(preset).trigger('change');
context.syncSyslogSearchBuilder();
assert.equal(nodes.rfilter.value, 'logtime last "604800"');
const dateOperator = builder.querySelector('.syslogSearchRow').children[2];
dateOperator.value = '>=';
