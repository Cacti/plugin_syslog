 ...
 * @param int|string $port        The database port.  Defaults to 3306
 * @param int|string $retries     The number a time the server should attempt to connect before failing
 ...
 *
 * @return PDO|false connection for success, or bool false for error
 */
function syslog_db_connect_real($host, $user, $pass, $db_name, $db_type, $port = '3306', $retries = 20, $db_ssl = false,
	$db_ssl_key = '', $db_ssl_cert = '', $db_ssl_ca = '') {
	return db_connect_real($host, $user, $pass, $db_name, $db_type, (int) $port, (int) $retries, $db_ssl, $db_ssl_key, $db_ssl_cert, $db_ssl_ca);
}

/**
 * syslog_db_close - closes the open connection
 *
 * @param PDO $syslog_cnn The connection object to connect to
 *
 * @return bool the result of the close command
 */
function syslog_db_close($syslog_cnn): bool {
 ...
 */
function syslog_db_execute($sql, $log = true): bool {
	global $syslog_cnn;
 ...
 *
 * @param string       $sql   The sql query to execute
 * @param array<int,mixed> $parms The sql params for the prepare
 * @param bool         $log   Whether to log error messages, defaults to true
 *
 * @return bool true for success, false for error
 */
function syslog_db_execute_prepared($sql, $parms = [], $log = true): bool {
	global $syslog_cnn;
 ...
 *
 * @param string       $sql      The sql query to execute
 * @param array<int,mixed> $params   An array of parameters
 * @param string       $col_name Use this column name instead of the first one
 * @param bool         $log      Whether to log error messages, defaults to true
 *
 * @return mixed The output of the sql query as a single variable
 ...
 * @param bool   $log Whether to log error messages, defaults to true
 *
 * @return array<string, mixed>|false The first row of the result as a hash
 */
 ...
 * @param bool   $log Whether to log error messages, defaults to true
 *
 * @return list<array<string, mixed>>|false The entire result set as a multi-dimensional hash
 */
 ...
 */
function syslog_db_fetch_insert_id(): int {
	global $syslog_cnn;
 ...
 * @param array<string, mixed>  $array_items An array containing each column -> value mapping in the row
 * @param string|array<string>  $keyCols     The name of the column containing the primary key
 *
 * @return int|false The auto incriment id column (if applicable), or false for error
 */
 ...
 *
 * @param array<string, mixed>|string $array_items An array containing each column -> value mapping in the row
 * @param string                      $table_name  The name of the table to make the replacement in
 * @param string|array<string>        $key_cols    The primary key(s)
 * @param bool                        $autoinc     Is the primary key autoinc
 *
 * @return int|false The auto incriment id column (if applicable)
 */
 ...
 */
function syslog_db_table_exists($table, $log = true): bool {
	global $syslog_cnn;
	...
	if ($matches !== false && array_key_exists('table', $matches)) {
		...
		return (bool) db_fetch_cell($sql, '', $log, $syslog_cnn);
	}
...
/**
 * syslog_db_column_exists - checks whether a column exists in a table
 *
 * @param string $table  The name of the table
 ...
 */
function syslog_db_column_exists($table, $column, $log = true): bool {
	global $syslog_cnn;
...
/**
 * syslog_db_add_column - adds a column to a table
 *
 * @param string       $table  The name of the table
 ...
 */
function syslog_db_add_column($table, $column, $log = true): bool {
	global $syslog_cnn;
 ...
 *
 * @return int|false The number of rows affected by the last transaction, or false on error
 */
