...
function syslog_get_report_records() {
	global $syslog_cnn;
...
function syslog_report() {
	...
	if (isset($_REQUEST["clear_x"])) {
		...
		kill_session_var("sess_syslog_report_rows_selector");
		...
		$changed += check_changed("rows_selector", "sess_syslog_report_rows_selector");
	...
	load_current_session_value("rows_selector", "sess_syslog_report_rows_selector", "20");
	...
	/* generate page list */
	$url_page_select = get_page_list($_REQUEST["page"], MAX_DISPLAY_PAGES, $_REQUEST["rows_selector"], $total_rows, "syslog_reports.php");
					...
					<table width='100%' cellspacing='0' cellpadding='0' border='0'>
						<tr>
							...
							<td align='right' class='textHeaderDark'>
								<strong>"; if (($_REQUEST["page"] * $_REQUEST["rows_selector"]) < $total_rows) { $nav .= "<a class='linkOverDark' href='" . $config['url_path'] . "plugins/syslog/syslog_reports.php?page=" . ($_REQUEST["page"]+1) . "'>"; } $nav .= "Next"; if (($_REQUEST["page"] * $_REQUEST["rows_selector"]) < $total_rows) { $nav .= "</a>"; } $nav .= " &gt;&gt;</strong>
							</td>\n
						</tr>
					</table>
				</td>
